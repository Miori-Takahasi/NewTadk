...
public final class EmployeeManagementController extends BaseServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Tips: 定義済みフィールド変数を使用
        // [ここへ記述]
        super.ems = new EmployeeManagementService();
        ...
        try {
            ...
            super.responseBean = e.getResponseBean();
            Logger.log(e);
        } finally {

            Logger.log(new Throwable(), "reqStatus  = " + super.responseBean.getRequestStaus());
            Logger.log(new Throwable(), "reqMessage = " + super.responseBean.getMessage());

            request.setAttribute(CONST_REQUST_KEY_FOR_RESPONSE_BEAN, super.responseBean);

            Logger.log(new Throwable(), "遷移先 = " + super.destinationTarget);

            if (hasSession) {
                super.getServletContext().getRequestDispatcher(super.destinationTarget).forward(request, response);
            }
            super.ems = null;
            super.responseBean = null;
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        super.responseBean = new ResponseBean();
        ...
        try {
            requestType = super.getRequestType(request);
            Logger.log(new Throwable(), "RequestType = " + requestType.toString());
    ...
    private RequestType getRequestType(final HttpServletRequest request) throws MVCException {
        ...
        if (Objects.isNull(super.responseBean)) super.responseBean = new ResponseBean();

        try {
            if (Objects.isNull(requestTypeName)) {
                // この処理に到達してしまう場合は、リクエスト時に押下したボタン（疑似ボタン）の「name」、「value」をチェック
                super.responseBean.setRequestStaus(1);
                super.responseBean.setEmplyeeBeanList(new ArrayList<>(0));
                super.responseBean.setMessage(ConstMessage.ERROR_UNKNOWN_REQUEST);
            } else {
                // RequestTypeに未存在のキー値が指定された場合は例外発生
            ...
            Logger.log(new Throwable(), "super.responseBean = " + super.responseBean);

            // 何らかの理由でrequestTypeがnull、または、リクエストステータス > 0（エラーあり）の場合
            if (Objects.isNull(requestType) || super.responseBean.getRequestStaus() > 0) {
                // リクエスト対象の画面へ戻す
                super.destinationTarget = request.getParameter(CONST_REQUST_KEY_FOR_SENDER);
                throw new MVCException(super.responseBean);
            }
        }
