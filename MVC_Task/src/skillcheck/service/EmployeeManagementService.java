...
// FIXME Step-5-1: 「EmployeeBean, ResponseBean, ConstMessage, ConstSQL, EmployeeDao, MVCException, Logger」をインポートしなさい。
// [ここへ記述]
...
public final class EmployeeManagementService extends BaseService implements EmployeeDao {
    ...
    public ResponseBean getEmployeeData(final ExecuteCase eCase) throws MVCException {
        ...
        super.responseBean = this.executeDBAccess(eCase, new ArrayList<>(0));
    ...
    public ResponseBean getEmployeeData(final ExecuteCase eCase, EmployeeBean pEmployeeBean) throws MVCException {
        ...
        super.responseBean = this.executeDBAccess(eCase, Arrays.asList(pEmployeeBean));
        ...
        try {
            ...
            while (super.resultSet.next()) {
                ...
                while (i < meta.getColumnCount()) {
                    i++;
                    Logger.log(new Throwable(), meta.getColumnName(i) + ": " + super.resultSet.getString(meta.getColumnName(i)));
                }
            ...
            Logger.log(new Throwable(), "reqStatus  = " + super.reqStatus);
            Logger.log(new Throwable(), "reqMessage = " + super.reqMessage);

            if (super.reqStatus == 0) {
                switch (eCase) {
                case FIND_ALL:
                case FIND_BY_EMPID:
                case FIND_BY_EMPID_WITH_LIKE:
                    ...
                    super.reqMessage = String.format(ConstMessage.SUCCECSS_RECORD_COUNT, count);
                    employeeBean = null;
                    break;
                default:
                    break;
            ...
            super.responseBean.setEmplyeeBeanList(empResultList);
            super.responseBean.setRequestStaus(super.reqStatus);
            super.responseBean.setMessage(super.reqMessage);

            super.executeCloseSQL();

            empResultList = null;

            if (super.reqStatus == 2) throw new MVCException(super.responseBean);
        }
    ...
    private void executeSelectQuery(final ExecuteCase eCase, final List<EmployeeBean> pEmployeeBeanList)
        ...
        try {
            ...
            case FIND_ALL:
                ...
                super.resultSet = super.connection.createStatement().executeQuery(sbQuery.toString());
